...
class CRC16:
    ...
    BLUFI_DEFAULT_INIT = 0x0000
    BLUFI_DEFAULT_XOROUT = 0xFFFF

    @staticmethod
    def calculate(data: str, init_crc: int = BLUFI_DEFAULT_INIT, xor_out: int = BLUFI_DEFAULT_XOROUT) -> str:
        """
        calculate CRC16.
        :param data:
        :param init_crc:
        ...
        byte_list = [int.from_bytes(bytes.fromhex(data[i : i + 2]), byteorder="little") for i in range(0, len(data), 2)]
        try:
            byte_iter = (byte for byte in byte_list)
        except TypeError:
            raise ValueError("数据中包含非整数元素，无法解析为字节")
        crc = init_crc
        crc ^= xor_out
